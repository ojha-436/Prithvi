...
export default function Insights() {
    ...
    return (
      <div className="animate-fade-up">
        <PageHeader
          ...
          description="Personalised actions appear after you log a footprint."
        />
  ...
  const potentialSaving = (recs ?? [])
    .filter((r) => !pledged.has(r.id))
    .reduce((s, r) => s + r.estimatedSavingKg, 0);
    ...
    <div className="animate-fade-up space-y-6">
      <PageHeader
        ...
        action={
          <Badge variant="muted" className="h-8 gap-1.5 px-3">
            ...
            {isGeminiConfigured ? "Powered by Gemini" : "Prithvi engine"}
          </Badge>
        }
      />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard
          ...
          tone="success"
        />
        <StatCard icon={Trophy} label="Points earned" value={formatNumber(game.points)} />
        <StatCard
          icon={Target}
          label="Still available"
          value={`${formatCO2(potentialSaving)}/yr`}
          tone="accent"
        />
      </div>
            ...
            >
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{
                    background: `${CATEGORY_META[rec.category].color}1f`,
                    color: CATEGORY_META[rec.category].color,
                  }}
                >
              ...
              <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
                <span className="text-sm">
                  ...
                  <strong className="tabular text-success">
                    {formatCO2(rec.estimatedSavingKg)}
                  </strong>
                  /yr
                </span>
                <Button
                ...
                >
                  {done ? (
                    <>
                      <Check className="size-4" /> Pledged
                    </>
                  ) : (
                    "I'll do this"
                  )}
            ...
            <ul className="divide-y divide-border">
              {game.completedPledges.map((p) => (
                <li key={p.id} className="flex items-center gap-3 py-3 text-sm">
                  <span className="bg-success/12 grid size-7 place-items-center rounded-full text-success">
                    <Check className="size-4" />
                  </span>
                  <span className="flex-1">{p.title}</span>
                  <span className="tabular font-medium text-success">
                    {formatCO2(p.savingKg)}/yr
                  </span>
                </li>
              ))}
            </ul>
